...
static inline void Rans64EncPut(uint64_t* r, std::vector<uint32_t>& vec, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
...
// Flushes the rANS encoder.
static inline void Rans64EncFlush(uint64_t* r, std::vector<uint32_t>& vec)
{
    uint64_t x = *r;

    vec.push_back((uint32_t) (x >> 32));
    vec.push_back((uint32_t) (x >> 0));
}
...
// Initializes a rANS decoder.
// Unlike the encoder, the decoder works backwards.
static inline void Rans64DecInit(uint64_t* r, std::vector<uint32_t>& vec)
{
    uint64_t x;

    x  = (uint64_t) vec.back() << 0;
    vec.pop_back();
    x |= (uint64_t) vec.back() << 32;
    vec.pop_back();
    *r = x;
}
...
// "start" and frequency "freq". All frequencies are assumed to sum to "1 << scale_bits",
// and the resulting bytes get written to ptr (which is updated).
static inline void Rans64DecAdvance(Rans64State* r, std::vector<uint32_t>& vec, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    uint64_t mask = (1ull << scale_bits) - 1;
